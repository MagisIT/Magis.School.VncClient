...
class ZRLEEncoding : FrameEncoding(encodingId = 16) {
    ...
    var tileX: Int = 0
    var mode: Byte = 0
    var isRunLengthEncoded: Boolean = false
    var paletteSize: Int = 0
    ...
    override fun readAndDecode(
        ...
        maxUncompressedLength = width * height * 3 + ((width % 64 + 1) * (height % 64 + 1) * 10000)
        ...
        while (tileY < yPosition + height) {
            ...
            tileHeight = min(yPosition + height - tileY, 64)
            ...
            while (tileX < xPosition + width) {
                ...
                tileWidth = min(xPosition + width - tileX, 64)
                ...
                if (!isRunLengthEncoded) {
                    ...
                    when {
                        paletteSize == 0 -> {
                            ...
                            repeat(tileHeight) { curY ->
                                repeat(tileWidth) { curX ->
                                    ...
                                    pixels[width * (tileY - yPosition + curY) + tileX - xPosition + curX] =
                        ...
                        paletteSize == 1 -> {
                            // Palette size is one which means that the tile has one color which the server sends

                            // Set the color
                            currentColor = cPixelPalette[0]
                                ...
                                paletteSize == 2 -> {
                                    ...
                                    repeat(packedPixelSize) { index ->
                                        ...
                                        for (i in 7 downTo 0) {
                                            ...
                                            pixels[width * (tileY - yPosition + currentTileY) + tileX - xPosition + currentTileX] =
                ...
                // Continue with the next tile (tiles are always 64 pixels wide but can we smaller if they are at the end)
                tileX += 64
            }
            // Continue with the next tile (tiles are always 64 pixels wide but can we smaller if they are at the end)
            tileY += 64
